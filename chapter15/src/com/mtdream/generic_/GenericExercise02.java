    ...
    public static void main(String[] args) {
        ...
        employees.sort(new Comparator<Employee>() {
            ...
            public int compare(Employee emp1, Employee emp2) {
                ...
                //先对传入的参数进行验证
                //确保emp1和emp2的类型是Employee，如果不是则返回0
                if(!(emp1 instanceof  Employee && emp2 instanceof Employee)) {
                    System.out.println("类型不正确..");
                    return 0;
                }
                // 如果是，则开始比较name，首先要先获取到name，用到getName方法
                // 然后比较名字是否相同，运用compareTo（）方法，它会返回一个int i值
                // 如果不相同则i不等于0
                int i = emp1.getName().compareTo(emp2.getName());
                if(i != 0) {
                    return i;
                }
                //如果name相同，就比较 birthday的year
//                int yearMinus = emp1.getBirthday().getYear() -emp2.getBirthday().getYear();
//                if(yearMinus != 0){
//                    return yearMinus;
//                }
                //如果year相同，就比较 birthday的month
